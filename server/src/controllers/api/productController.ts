...
// cursor based pagination but as Offset Pagination
export const getProductsByPagination = [
  ...
  async (req: CustomRequest, res: Response, next: NextFunction) => {
    ...
    // If "type" exists, convert it into an array of valid numeric type IDs
    if (type) {
      typeList = type
        .toString() // Ensure the value is treated as a string
        .split(",") // Split comma-separated string into an array
        .map((type) => Number(type)) // Convert each item into a number
        .filter((t) => t > 0); // Keep only positive numbers (Integer)
    }
